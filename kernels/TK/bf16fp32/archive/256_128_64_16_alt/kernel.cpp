...
constexpr int REG_BLOCK        = BLOCK_SIZE_N / 4;
...
void micro_tk(const micro_globals g) {
    ...
    rt_bf<REG_BLOCK, DOT_SLICE> a_reg[2];
    ...
    load(a_reg[0], subtile_inplace<REG_BLOCK, DOT_SLICE>(As, {warp_row, 0}));
    ...
    for (int tile = 0; tile < num_tiles; ++tile) {
        ...
        load_async_shared_to_register(a_reg[1], subtile_inplace<REG_BLOCK, DOT_SLICE>(As, {warp_row, 1}));

        __builtin_amdgcn_s_setprio(3);
        mma_ABt(C_accum, a_reg[0], b_reg[0], C_accum);
        ...
        load(a_reg[0], subtile_inplace<REG_BLOCK, DOT_SLICE>(As, {warp_row, 2}));

        __builtin_amdgcn_s_setprio(3);
        mma_ABt(C_accum, a_reg[1], b_reg[1], C_accum);
        __builtin_amdgcn_s_setprio(0);
        asm volatile("s_waitcnt lgkmcnt(0)");

        // Iteration 2
        load_async_shared_to_register(b_reg[1], subtile_inplace<REG_BLOCK, DOT_SLICE>(Bs, {warp_col, 3}));
        load_async_shared_to_register(a_reg[1], subtile_inplace<REG_BLOCK, DOT_SLICE>(As, {warp_row, 3}));
    ...
    rt_bf<REG_BLOCK, REG_BLOCK, ducks::rt_layout::col> C_accum_bf16;

    copy(C_accum_bf16, C_accum);
    store(g.c, C_accum_bf16, {
        0, 0,
        row * 2 + warp_row,
        col * 4 + warp_col
    });
