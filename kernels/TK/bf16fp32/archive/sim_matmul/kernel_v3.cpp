...
void micro_tk(const micro_globals g) {
    ...
    rt_bf<REG_BLOCK, K_STEP> a_reg;
    rt_bf<K_STEP, REG_BLOCK, ducks::rt_layout::col> b_reg;
    ...
    for (int tile = 0; tile < num_tiles; ++tile) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            auto st_sub_a = subtile_inplace<REG_BLOCK, K_STEP>(As, {subtile_row, 0});
            auto st_sub_b = subtile_inplace<K_STEP, REG_BLOCK>(Bs, {0, subtile_col});
            load(a_reg, st_sub_a);
            load(b_reg, st_sub_b);
