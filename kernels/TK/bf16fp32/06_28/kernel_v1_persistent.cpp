...
__device__ static inline int2 get_task_idx(const micro_globals &g, int task_iter) {
    const int total_tiles = (M / BLOCK_SIZE) * (N / BLOCK_SIZE);
    const int tile_idx = blockIdx.x + task_iter * gridDim.x;

    if (tile_idx >= total_tiles) return {-1, -1};  // signal done

    const int row = tile_idx / (N / BLOCK_SIZE);
    const int col = tile_idx % (N / BLOCK_SIZE);
    return {row, col};
}
...
void micro_tk(const micro_globals g) {
    ...
    for(int task_iter = 0; true; task_iter++) {
        int2 rowcol = get_task_idx(g, task_iter);
        const int row = rowcol.x;
        const int col = rowcol.y;
        if(row == -1) { break; }
