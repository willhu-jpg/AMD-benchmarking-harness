import torch
# import kernel_baseline as tk_kernel
import random
import time
import argparse
...
def load_module_from_so(so_path):
    # Extract the base filename and remove everything after the first dot
...
parser.add_argument("modules", nargs="+", help="List of .so files to run")
...
for idx, so_path in enumerate(args.modules):
    print(f"\n\n--- Running module: {so_path} ---")
    tk_kernel = load_module_from_so(so_path)
