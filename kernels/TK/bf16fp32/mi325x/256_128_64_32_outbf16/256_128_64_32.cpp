...
__device__ inline int2 get_task_idx(int task_id) {
    int wgid = task_id; 

    const int total_tiles = (M / BLOCK_SIZE_M) * (N / BLOCK_SIZE_N);
    if (task_id >= total_tiles) return {-1, -1};  // signal done

    const int NUM_WGS = (N / BLOCK_SIZE_N) * (M / BLOCK_SIZE_M);
...
void micro_tk(const micro_globals g) {
    ...
    for(int task_iter = 0; true; task_iter++) {
        ...
        int task_id = blockIdx.x + task_iter * gridDim.x;
        int2 rowcol = get_task_idx(task_id);
        if(rowcol.x == -1) { break; }
