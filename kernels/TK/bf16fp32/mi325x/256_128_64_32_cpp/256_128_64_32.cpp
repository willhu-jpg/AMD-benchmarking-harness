...
struct micro_globals {
    ...
    dim3 grid()  { return dim3((MATRIX_N / BLOCK_SIZE_N) * (MATRIX_M / BLOCK_SIZE_M)); } 
    dim3 block() { return dim3(NUM_THREADS); } 
    size_t dynamic_shared_memory() { return 49152; } //49152; }
};
...
void micro_tk(const micro_globals g) {
    ...
    const int num_tiles = MATRIX_K / K_STEP;
    ...
    const int WGM = 16;
    const int num_pid_m = ceil_div(MATRIX_M, BLOCK_SIZE_M);
    const int num_pid_n = ceil_div(MATRIX_N, BLOCK_SIZE_N);
    ...
    rt_bf<REG_BLOCK, REG_BLOCK, ducks::rt_layout::col> C_accum_bf16[4];
    #pragma unroll
    for (int i = 0; i < 4; ++i) {
...
void matmul(__hip_bfloat16* A, __hip_bfloat16* B, __hip_bfloat16* C) {
    ...
    _gl_A a_arg{A, 1, 1, MATRIX_M, MATRIX_K};
    _gl_B b_arg{B, 1, 1, MATRIX_N, MATRIX_K};
    _gl_C c_arg{C, 1, 1, MATRIX_M, MATRIX_N};
