extern "C" __global__ void matmul_kernel(int M, int N, int K, float *A, float *B, float *C, float alpha, float beta) {
    ...
    for (int i = 0; i < steps; i++) {
        ...
        Bs[((sB_col % 2) * 4 + sB_row * 8 + 0) * 16 + sB_col / 2] = tmp.x;
        Bs[((sB_col % 2) * 4 + sB_row * 8 + 1) * 16 + sB_col / 2] = tmp.y;
        Bs[((sB_col % 2) * 4 + sB_row * 8 + 2) * 16 + sB_col / 2] = tmp.z;
        Bs[((sB_col % 2) * 4 + sB_row * 8 + 3) * 16 + sB_col / 2] = tmp.w;
        __syncthreads();
