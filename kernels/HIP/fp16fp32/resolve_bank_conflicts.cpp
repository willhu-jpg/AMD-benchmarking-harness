...
extern "C" __global__ void matmul_kernel(int M, int N, int K, __half *A, __half *B, float *C, float alpha, float beta) {
    ...
    for (int i = 0; i < steps; i++) {
        ...
        Bs[((sB_col % 2) * 4 + sB_row * 8 + 0) * 16 + sB_col / 2] = tmp1.x;
        Bs[((sB_col % 2) * 4 + sB_row * 8 + 1) * 16 + sB_col / 2] = tmp1.y;
        Bs[((sB_col % 2) * 4 + sB_row * 8 + 2) * 16 + sB_col / 2] = tmp2.x;
        Bs[((sB_col % 2) * 4 + sB_row * 8 + 3) * 16 + sB_col / 2] = tmp2.y;
        __syncthreads();
