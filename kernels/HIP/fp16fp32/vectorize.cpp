...
extern "C" __global__ void matmul_kernel(int M, int N, int K, __half *A, __half *B, float *C, float alpha, float beta) {
    ...
    for (int i = 0; i < steps; i++) {
        ...
        reinterpret_cast<__half2 *>(&Bs[sB_row * BN + sB_col * 4])[0] = 
            reinterpret_cast<__half2 *>(&B[sB_row * N + sB_col * 4])[0];
        reinterpret_cast<__half2 *>(&Bs[sB_row * BN + sB_col * 4 + 2])[0] = 
            reinterpret_cast<__half2 *>(&B[sB_row * N + sB_col * 4 + 2])[0];
        __syncthreads();
